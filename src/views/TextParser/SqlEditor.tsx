...
const SqlEditor: React.FunctionComponent<SqlEditorProps> = () => {
  const [value, setValue] = React.useState("insert into table_str(name, age, hobby, time) values(?,?,?,now());");
  ...
  const onChange = useCallback((val: string, viewUpdate: any) => {
    ...
    setValue(val);
    const parseSqlObject = parseInsertFunc(val);
        ...
        {tableStatus ? <>
          <table className=" divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr key={'header'}>
                {sqlColumns.length > 0 ? sqlColumns.map((item, index) => (
                  <th key={index} className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{item as string}</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              <tr key={'row'} className="hover:bg-gray-50">
                {sqlValues.length > 0 ?
                  sqlValues.map((item: any, index) => (item.type == 'origin' ?
                    <td key={index} className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{item.value}</td> : <td key={index}>no</td>))
